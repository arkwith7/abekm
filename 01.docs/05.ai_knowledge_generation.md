# 05. IP Agent 및 지식생성 시스템 상세 설계

> 본 문서는 IPBridge 플랫폼 내 **IP Agent 및 지식생성** 서브시스템의 설계를 기술합니다. 기존의 범용 AI Agent를 IP 전문 Agent(선행기술 조사, 청구항 생성 등)로 고도화하는 내용을 포함합니다.
...
- **IP Copilot** 기반의 대화형 특허 분석 및 생성 지원
- **선행기술 조사 Agent**: KIPRIS + 내부 DB 하이브리드 검색 및 X-Y 매트릭스 생성
- **청구항 생성 Agent**: 발명 포인트 식별 및 청구항 초안 자동 작성
- **특허 가치평가 Agent**: 실물옵션 기반 가치 산정 (Planned)

## 2. 핵심 사용자 시나리오

| 시나리오          | 설명                              | 주요 컴포넌트                                   |
| ------------- | ------------------------------- | ----------------------------------------- |
| 1. **선행기술 조사** | 발명 아이디어 입력 → 유사 특허 검색 → X-Y 매트릭스 | PriorArtAgent, KIPRIS Client, SBERT |
| 2. **청구항 생성** | 선행기술 차별점 분석 → 청구항 초안 작성 | ClaimGenerationAgent, LLM |
| 3. 문서 세트 질문   | 다수 문서를 선택 후 질의                  | ChatPage.tsx, RAGSearchService            |
| 4. PPT 자동 생성  | 대화 메시지 기반 아웃라인/슬라이드 생성          | Presentation tools, SSE stream            |
| 5. 실시간 질의응답   | 검색 + 생성 답변 스트리밍                 | /api/v1/chat/stream, EventSource          |
| 6. 에이전트 모드 전환 | General / Analyst (IP 전문) 모드 선택 | useWorkContext, Toolbar UI                |
...
IPBridge은 AWS Transcribe Streaming을 활용한 실시간 음성인식 기능을 제공하여 음성으로 AI 채팅을 이용할 수 있습니다.
...
| `partial_results_stability` | **high** | **low**: 빠른 응답, 잦은 변경 (타이핑 느낌)<br>**medium**: 균형 (기본값)<br>**high**: 느린 응답, 높은 정확도 (추천). 의료/기술 용어가 많은 IPBridge 특성상 잦은 변경보다 정확한 초기 추측이 중요 |
