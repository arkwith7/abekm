...
|------|--------|----------------|
| `energyThreshold` | **0.02** | Pre-roll 버퍼링 단계에서는 필터링 비활성화 (초기 발화를 무조건 캡처). 실시간 전송 단계(isConnected=true)에서만 필터 적용하여 AWS로 전송되는 데이터량 감소 |
| `maxSilenceFrames` | **30개** | 약 3.8초 연속 침묵 시 전송 중단. Pre-roll에는 적용되지 않으며, 실시간 전송 중 사용자가 말을 멈춘 후 배경 소음만 전송되는 것 방지 |

#### 효과 검증

**테스트 케이스**:
- 발화: "인슐린 펌프에 대해 소개해 주세요"
- 예상 결과: 전체 문장 인식 ✅
- 실제 결과 (수정 전): "에 대해 소개해 주세요" (첫 단어 손실 ❌)
- 실제 결과 (수정 후): "인슐린 펌프에 대해 소개해 주세요" (완전 인식 ✅)

### 6.2.4 WebSocket 프로토콜
