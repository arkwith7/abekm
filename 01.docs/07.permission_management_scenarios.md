# 07. ABEKM 권한 관리 시나리오 상세 설명서

## 1. 개요

본 문서는 ABEKM 시스템에서 지식맵 구조에 따른 권한 관리 체계와 각 역할별 권한 관리 시나리오를 상세히 설명합니다.
...
- **상향 상속**: 하위 컨테이너 권한이 상위 컨테이너로 자동 상속되지 않음
- **하향 상속**: 상위 컨테이너 권한이 하위 컨테이너로 선택적 상속 가능
- **직접 권한**: 특정 컨테이너/문서에 직접 부여된 권한이 최우선

## 3. 지식생성자 (Knowledge Creator) 권한 관리 시나리오

### 3.1 지식파일 업로드 시 접근 범위 설정
...
enum AccessScope {
  OWNER_ONLY = 'owner',           // 나만 보기
  SAME_TEAM = 'team',             // 같은 팀 보기
  SAME_DEPARTMENT = 'department', // 같은 부서 보기
  SAME_DIVISION = 'division',     // 같은 본부 보기
  ALL_USERS = 'company',          // 전사 보기
  PUBLIC = 'public'               // 외부 공유 가능
}
```

#### 3.1.2 지식생성자 파일 업로드 프로세스

**📤 시나리오 1: 채용팀 직원이 "채용가이드.pdf" 업로드**
...
INSERT INTO tb_file_bss_info (
    ...
    100, '채용가이드', 'recruit_guide_20250128.pdf', 'pdf', 
    '/storage/hr/recruit/', 'emp001'
);

-- 2단계: 파일 상세 정보 저장 (보안 등급 포함)
INSERT INTO tb_file_dtl_info (
    SJ, AUTHR, CTGRY_CD, SECRTY_LVL, CREATED_BY
) VALUES (
    '2025년 채용 가이드라인', '김채용', 'HR_RECRUIT', 'TEAM', 'emp001'
);

-- 3단계: 컨테이너별 권한 자동 설정
-- 지식생성자가 선택한 접근 범위에 따라 자동 권한 부여
```

**🎯 지식생성자 권한 설정 옵션:**

| 설정 옵션     | 접근 범위      | 자동 권한 부여 대상 | 예시 시나리오      |
| --------- | ---------- | ----------- | ------------ |
| **나만 보기** | PRIVATE    | 본인만         | 개인 작업 문서, 초안 |
| **팀 공유**  | TEAM       | 채용팀 전체      | 팀 내부 업무 매뉴얼  |
| **부서 공유** | DEPARTMENT | 인사팀 전체      | 부서 정책 문서     |
| **본부 공유** | DIVISION   | 경영지원본부 전체   | 본부 차원 가이드라인  |
...
class CreatorPermissionManager:
    def update_document_permissions(self, document_id: str, creator_id: str, new_permissions: DocumentPermissions):
        """지식생성자가 자신이 업로드한 문서의 권한 수정"""
        
        # 1. 생성자 권한 확인
        if not self.is_document_creator(document_id, creator_id):
            raise PermissionDenied("문서 생성자만 권한을 수정할 수 있습니다.")
...
## 4. 지식관리자 (Knowledge Manager) 권한 관리 시나리오
...
│                                                 │
│  🔄 권한 변경 요청 (2건):                           │
│  • 이영업 (영업팀) - 읽기 권한 요청    [승인/거부]    │
│  • 마케팅팀 - 팀 전체 읽기 권한 요청   [승인/거부]    │
│                                                 │
│  📊 접근 통계 (최근 30일):                          │
│  조회: 47회  다운로드: 12회  댓글: 3개             │
│                                                 │
│        [권한 추가]    [대량 권한 수정]    [닫기]     │
└─────────────────────────────────────────────────┘
```
...
def create_project_permissions(self, project_info: ProjectInfo):
    ...
    # 3. 프로젝트 종료 시 자동 권한 정리 스케줄링
    self.schedule_permission_cleanup(project_container.id, project_info.end_date)
```

이 상세한 권한 관리 시나리오를 통해 ABEKM 시스템에서는 지식생성자와 지식관리자가 각자의 역할에 맞는 세분화된 권한 관리를 수행할 수 있으며, SAP 조직도와 연동하여 자동화된 권한 관리가 가능합니다.

## 8. 통합 권한 관리 API 구조 (v1.0 리팩토링)
...
- **단일 파일 관리**: `permissions.py`에서 권한 관리와 요청 워크플로우 통합
- **일관된 인증**: 모든 권한 API에서 동일한 토큰 기반 인증 체계
- **역할 기반 접근**: 관리자 권한 확인을 통한 세분화된 권한 제어
- **감사 로깅**: 모든 권한 변경 사항에 대한 상세 로그 기록

#### 📈 운영 효율성

- **통합 관리**: 권한 관리자가 단일 API 세트로 모든 권한 관리 업무 수행
- **자동화 지원**: 권한 요청 승인 프로세스의 자동화 기반 마련
- **모니터링**: 실시간 권한 현황 및 요청 상태 추적
- **확장성**: 향후 고급 권한 관리 기능 추가를 위한 견고한 기반 구축