...
본 문서는 IPBridge(AI Based Enterprise Knowledge Management)의 정적 설계를 정의합니다. 현재 구현된 SQLAlchemy 모델들을 기반으로 주요 객체들의 구조, 속성, 관계를 표 형태로 상세히 기술합니다.
...
| **인증/권한**  | User                        | tb_user                      | IPBridge 사용자 인증          |
...
### 3.2 User (IPBridge 사용자)

**테이블명**: `tb_user`  
**목적**: IPBridge 시스템 사용자 인증 및 기본 정보 관리
...
이 정적 설계 명세서는 IPBridge 시스템의 **실제 운영 중인 데이터베이스 스키마 (2025-10-27 기준)**를 PostgreSQL 직접 조회 및 SQLAlchemy 모델 분석을 통해 완전히 문서화했습니다.
