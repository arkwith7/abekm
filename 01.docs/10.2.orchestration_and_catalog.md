...
- `backend/app/agents/core/workers.py`: 기본 워커 정의(Phase 2에서 feature discovery로 확장 예정)
...
현재 `get_workers()`는 legacy 기본 워커 목록을 반환합니다.
Phase 2에서는 아래로 확장합니다.
- `agents/features/*`를 우선 탐색해 WorkerSpec을 발견
- (선택) capability 태그 기반 discovery 제공
