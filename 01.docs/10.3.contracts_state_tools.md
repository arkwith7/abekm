...
현재 코드 기준(표준 계약 위치):
- 공용 계약 타입은 `backend/app/core/contracts.py`에 정의되어 있으며, Tool/Agent 구현은 이를 import하여 사용합니다.
	- 예: `ToolResult`, `SearchChunk`, `AgentIntent`, `AgentConstraints` 등
...
구현 메모:
- 검색 계열 도구는 `SearchChunk`를 표준 결과 단위로 사용하도록 정리되어 있으며, 상위 에이전트가 이를 컨텍스트/답변 근거로 재사용합니다.
