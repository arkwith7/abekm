...
> 목적: 좌측 메뉴(대시보드/IP 포트폴리오/IP Copilot/선행기술 조사/대화 이력/탐색) 이동 시 **화면 상태를 즉시 복원**하고, 새로고침 시에는 **필요한 최소 상태만 복원**하여 저장 폭/리렌더/혼란을 줄인다.
...
- **Persist (새로고침 복원용)**: `IPBridge-app-state` (Zustand persist)
...
| **IP 포트폴리오(내 지식)** | 선택 컨테이너, 확장 트리, 페이지/정렬/필터, 로컬 목록 캐시(컨테이너/문서) | 선택 컨테이너/확장/페이지/정렬/필터/선택문서 | 컨테이너/문서 목록(새로고침 시) | 메뉴 이동: 즉시 복원 / 새로고침: 최신 재조회 |
| **IP Copilot(채팅)** | 선택 문서, 세션 ID(있다면), 입력 UI 상태 | 선택 문서/세션 id 정도 | 세션 복원(필요 시) | `useAgentChat`은 별도의 localStorage 백업을 사용(정책 하단 참고) |
| **선행기술 조사(채팅)** | 선택 문서, 세션 ID(있다면), 입력 UI 상태 | 선택 문서/세션 id 정도 | 세션 복원(필요 시) | IP Copilot 채팅 UI 재사용, 기본 도구(prior-art)로 고정 |
...
- **Zustand 스토어 + persist(부분 저장)**: `frontend/src/store/globalAppStore.ts`
  - persist key: `IPBridge-app-state`
...
## IP Copilot/선행기술 조사(Agent Chat) 전용 보존 정책(추가)

현재 `useAgentChat`은 아래 키로 별도 백업/복원을 수행한다:

- `IPBridge_agent_chat_state` (페이지 재방문/새로고침 대비)
- `agent_session_{sessionId}` (세션 단위 백업)

구현 참고:

- 채팅 라우트는 `/user/agent-chat` 뿐 아니라 `/user/ip-copilot`, `/user/prior-art`에서도 동일하게 세션 백업/복원이 동작하도록 확장됨
...
- **새로고침**: 위 localStorage 백업을 사용하되,
  ...
  - 장기적으로는 `IPBridge-app-state`와 중복 저장을 피하도록 정리(선택)

## 운영 체크리스트(권장)

- **저장 폭 모니터링**: localStorage의 `IPBridge-app-state` 크기가 커지지 않는지 주기 점검
