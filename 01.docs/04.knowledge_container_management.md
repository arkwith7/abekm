...
> **최종 업데이트**: 2026-01-05  
> **시스템 범위**: 조직도 기반 일반 문서/지식 관리  
> **주요 특징**:
> - **조직도 기반 계층 구조**: 부서/팀 단위 지식 저장소
> - **SAP 조직도 연동**: 실시간 조직 변경 동기화
> - **범용 문서 관리**: 업무 매뉴얼, 기술 문서, 회의록, 가이드라인 등
> - **컨테이너 권한**: 부서별/프로젝트별 접근 제어
> 
> ⚠️ **IP 포트폴리오와 분리**: 특허 관리는 별도 시스템([17.IP_Portfolio_Complete_Guide.md](17.IP_Portfolio_Complete_Guide.md)) 참조

## 0. 시스템 위치 및 역할

### 지식 컨테이너 vs IP 포트폴리오

| 항목 | 지식 컨테이너 (본 문서) | IP 포트폴리오 (별도 시스템) |
|------|----------------------|---------------------------|
| **관리 대상** | 일반 업무 문서/지식 | 특허/IP 자산 |
| **분류 기준** | 조직도/부서/프로젝트 | IPC/CPC 기술 분류 |
| **권한 체계** | 컨테이너 기반 (부서/팀) | IPC 코드 기반 |
| **메타데이터** | 작성자, 날짜, 버전 | 출원번호, 법적상태, 청구항 |
| **UI 경로** | `/containers` | `/ip-portfolio` |
| **SAP 연동** | ✅ 필수 (조직도 동기화) | ❌ 불필요 |
| **용도** | 매뉴얼, 가이드, 회의록 | 특허, 논문, 기술 보고서 |

### 구현 현황 (2026-01-05)

#### ✅ 완료
- 데이터베이스 스키마 설계 (tb_knowledge_containers, tb_container_permissions)
- SAP RFC 연동 구조 정의
- 컨테이너 계층 구조 및 권한 상속 로직

#### 🚧 진행 중
- 백엔드 API 구현 (`/api/v1/containers`)
- 프론트엔드 컨테이너 트리 UI
- SAP 조직도 실시간 동기화
...
#### 사례 1: 부서별 업무 매뉴얼 관리
- **시나리오**: 기술본부 → R&D센터 → AI팀의 "모델 개발 가이드" 관리
- **분류**: 조직 계층 탐색 (`/회사/기술본부/R&D센터/AI팀`)
- **권한**: AI팀 소속 직원 자동 읽기 권한, 팀장 쓰기 권한
- **SAP 연동**: 조직 변경 시 자동 권한 업데이트

#### 사례 2: 횡단 프로젝트 지식 공간
- **시나리오**: "차세대 제품 개발" 프로젝트 (R&D + 영업 + 생산)
- **분류**: 프로젝트 컨테이너 (`/프로젝트/차세대제품개발`)
- **권한**: 프로젝트 멤버에게 명시적 권한 부여
- **특징**: 부서 경계를 넘는 협업 공간

#### 사례 3: 규정집 및 SOP 관리
- **시나리오**: 전사 품질관리 규정, ISO 문서
- **분류**: 전사 공통 컨테이너 (`/회사/품질관리/규정집`)
- **권한**: 전 직원 읽기, 품질팀 쓰기
- **버전 관리**: 규정 변경 이력 추적
...
IPBridge의 지식 컨테이너 관리 시스템은 **조직도 기반 계층 구조**를 통해 기업 내 일반 문서 및 지식을 체계적으로 관리하는 시스템입니다. SAP ERP의 조직도와 실시간 동기화하여 부서 변경, 팀 이동 등의 조직 개편 시에도 자동으로 권한 및 문서 접근 구조를 업데이트합니다.

### 1.2 지식 컨테이너 개념

- **조직도 기반 계층**: 회사 → 본부 → 부서 → 팀 → 프로젝트 단위 지식 저장소
- **SAP 조직도 동기화**: SAP RFC 인터페이스를 통한 실시간 조직 구조 반영
- **범용 문서 관리**: 업무 매뉴얼, 기술 가이드, 회의록, SOP, 규정집 등
- **컨테이너 권한 상속**: 상위 컨테이너 권한이 하위 컨테이너로 자동 상속
- **프로젝트 컨테이너**: 부서 경계를 넘는 횡단 프로젝트 지식 공간

### 1.3 시스템 범위

- **조직도 기반 컨테이너 구조**: SAP 조직도와 1:1 매핑
- **컨테이너 권한 관리**: 읽기/쓰기/관리 권한 및 상속 규칙
- **문서 버전 관리**: 문서별 수정 이력 및 롤백
- **검색 및 탐색**: 조직 계층 탐색, 전체 텍스트 검색, 태그 기반 필터링
- **통계 대시보드**: 부서별 문서 수, 최근 업데이트 현황, 인기 문서
...
│  ├─ /containers (컨테이너 트리 탐색)                         │
│  ├─ /containers/:id (컨테이너 상세)                          │
│  ├─ /containers/:id/documents (문서 목록)                    │
│  └─ /containers/stats (통계 대시보드)                        │
├──────────────────────────────────────────────────────────────┤
│  🔧 백엔드 API (FastAPI + PostgreSQL)                        │
│  ├─ /api/v1/containers (CRUD)                               │
│  ├─ /api/v1/containers/tree (계층 구조)                      │
│  ├─ /api/v1/containers/:id/permissions (권한 관리)           │
│  └─ /api/v1/containers/sync-sap (SAP 동기화)                │
├──────────────────────────────────────────────────────────────┤
│  💾 데이터베이스 (PostgreSQL)                                │
│  ├─ tb_knowledge_containers (컨테이너 마스터)                │
│  ├─ tb_container_permissions (컨테이너 권한)                 │
│  ├─ tb_knowledge_categories (지식 분류)                      │
│  └─ tb_file_bss_info (문서 정보)                            │
├──────────────────────────────────────────────────────────────┤
│  🔗 외부 연동                                                │
│  ├─ SAP ERP (RFC 인터페이스)                                │
│  │   └─ 조직도 동기화 (부서/팀 구조)                         │
│  └─ Azure Blob / AWS S3 (문서 스토리지)                      │
└──────────────────────────────────────────────────────────────┘

⚠️ 특허 문서는 IP 포트폴리오 시스템으로 별도 관리
```
...
-- tb_patent_metadata 최소 확장 (레거시 조직 컨테이너 참조 보존 + IPC 필터용)
...
CREATE TABLE tb_knowledge_containers (
    ...
    org_path TEXT, -- 계층 경로 (예: /ROOT/기술본부/R&D센터/AI팀)
    
    -- 컨테이너 메타데이터
    description TEXT,
    container_type VARCHAR(20) DEFAULT 'department', -- department, team, project
    knowledge_category VARCHAR(50), -- 주요 지식 분야
    
    -- 권한 설정 강화
    access_level VARCHAR(20) DEFAULT 'internal', -- public, internal, restricted, confidential
    default_permission VARCHAR(20) DEFAULT 'VIEWER', -- 기본 권한 레벨
    inherit_parent_permissions BOOLEAN DEFAULT true, -- 상위 컨테이너 권한 상속
    permission_inheritance_type VARCHAR(20) DEFAULT 'additive', -- additive, override
    
    -- 권한 관리자 설정
    container_owner VARCHAR(20), -- 컨테이너 소유자 (ADMIN 권한)
    permission_managers TEXT[], -- 권한 관리자 목록
    auto_assign_by_org BOOLEAN DEFAULT true, -- 조직도 기반 자동 권한 할당
    
    -- 승인 워크플로우 설정
    require_approval_for_access BOOLEAN DEFAULT false, -- 접근 시 승인 필요
    approval_workflow_enabled BOOLEAN DEFAULT false, -- 권한 요청 승인 워크플로우
    approvers TEXT[], -- 승인자 목록
    ...
    -- 권한 통계
    user_count INTEGER DEFAULT 0, -- 접근 권한이 있는 사용자 수
    permission_request_count INTEGER DEFAULT 0, -- 권한 요청 건수
    last_permission_update TIMESTAMP, -- 마지막 권한 변경 시간
...
CREATE TABLE tb_knowledge_categories (
    ...
    -- 자동 분류 지원
    keywords TEXT[], -- 자동 분류를 위한 키워드
    classification_rules JSONB, -- 자동 분류 규칙
...
CREATE TABLE tb_container_categories (
    ...
    -- 매핑 속성
    is_primary BOOLEAN DEFAULT false, -- 주요 카테고리 여부
    weight FLOAT DEFAULT 1.0, -- 카테고리 가중치
    auto_assigned BOOLEAN DEFAULT false, -- 자동 할당 여부
...
ADD COLUMN secondary_categories INTEGER[]; -- 보조 카테고리들

-- 검색 문서 테이블에도 컨테이너 정보 추가 (이미 추가됨)
...
class OrganizationHierarchyManager:
    ...
    async def get_container_hierarchy(self, 
        ...
        """사용자 권한 기반 컨테이너 계층 구조 조회"""
        
        if container_id:
            # 특정 컨테이너의 하위 구조 (권한 확인 포함)
            query = """
...
CREATE TABLE tb_container_permissions (
    ...
    source_container_id INTEGER, -- 상속받은 컨테이너 ID
    sap_role VARCHAR(50), -- SAP 역할
    
    -- 권한 제약
    restricted_categories INTEGER[], -- 제한된 카테고리
    time_restriction JSONB, -- 시간 제한 {"start": "09:00", "end": "18:00"}
    ip_restriction INET[], -- IP 제한
    
    -- 권한 메타데이터
    granted_by VARCHAR(20),
    granted_date TIMESTAMP DEFAULT now(),
    expires_date TIMESTAMP, -- 권한 만료일
    is_active BOOLEAN DEFAULT true
);
...
CREATE TABLE tb_role_permission_templates (
    ...
    auto_assign_containers TEXT[], -- 자동 할당 컨테이너 패턴
...
class KnowledgeClassifier:
    ...
    async def classify_with_ai(self, 
        ...
        prompt = f"""
        다음 문서 내용을 분석하여 가장 적절한 카테고리를 선택해주세요.
        ...
        분류 결과를 JSON 형식으로 제공해주세요:
        {{
            "primary_category": "카테고리명",
            "confidence": 0.85,
            "secondary_categories": ["보조카테고리1", "보조카테고리2"],
            "reasoning": "분류 이유"
        }}
        """
...
class KnowledgeMapAnalyzer:
    def __init__(self):
        self.db = get_database()
        self.graph_db = get_neo4j()  # 선택적 그래프 DB
...
CREATE TABLE tb_knowledge_access_log (
    ...
    -- 컨텍스트 정보
    search_query TEXT, -- 검색을 통한 접근인 경우
    referrer_document_id INTEGER, -- 참조 문서
    access_duration INTEGER, -- 접근 시간 (초)
...
async def create_container(
    ...
    if container_data.parent_container_id:
        ...
        if not has_permission:
            raise HTTPException(403, "상위 컨테이너 관리 권한이 없습니다")
...
async def classify_document(
    ...
    if not has_permission:
        raise HTTPException(403, "분류 권한이 없습니다")
...
- **Materialized Path**: 조직 경로 저장으로 빠른 계층 조회
- **인덱스 최적화**: 트리 구조 전용 인덱스
- **캐싱 전략**: 자주 조회되는 계층 구조 캐싱
- **지연 로딩**: 필요한 부분만 점진적 로딩

### 9.2 권한 확인 최적화

- **권한 캐싱**: Redis 기반 권한 결과 캐싱
- **배치 권한 확인**: 여러 리소스 권한 일괄 확인
- **권한 상속 최적화**: 효율적인 상속 관계 계산

## 10. 모니터링 및 분석

### 10.1 컨테이너 사용 분석

- **문서 업로드 통계**: 컨테이너별 문서 증가 추이
- **사용자 접근 패턴**: 조직별 지식 활용도
- **지식 품질 지표**: 문서 품질 및 활용도
- **협업 패턴**: 조직간 지식 공유 분석
            ...
            "findings": [
                {
                    ...
                    "user": "emp_1234",
                    "description": "사용자가 본인 부서 외 컨테이너에 접근",
                    "risk_level": "medium",
    ...
    async def create_container(self, container_data: dict):
        ...
        return {
            ...
            "initial_setup": {
                ...
                "knowledge_templates": ["기본문서양식", "업무매뉴얼양식"],
                ...
                "기업": {
                    ...
                    "children": {
                        "기획본부": {
                            "container_id": "C010",
                            "children": {
                                "전략기획팀": {"container_id": "C011"},
                                "사업기획팀": {"container_id": "C012"}
                            }
                        },
                        "인사본부": {
                            "container_id": "C020",
                            "children": {
                                "인사팀": {"container_id": "C021"},
                                "교육팀": {"container_id": "C022"}
                            }
                        }
                    }
                }
...
class ContainerKnowledgeAnalyzer:
    async def analyze_container_knowledge(self, container_id: str):
        ...
        return {
            ...
            "usage_patterns": {
                "most_accessed": [
                    {"title": "신입사원 교육 매뉴얼", "access_count": 156},
                    {"title": "프로젝트 관리 가이드", "access_count": 134}
                ],
                "search_keywords": ["교육", "매뉴얼", "프로세스", "가이드"],
                "user_engagement": {
                    "active_users": 23,
                    "avg_session_time": "8.5분",
                    "return_rate": "78%"
                }
            },
            "knowledge_gaps": [
                {"area": "리모트워크 정책", "urgency": "high"},
                {"area": "신기술 도입 프로세스", "urgency": "medium"}
            ]
        }
```
...
class ContainerUserSupport:
    async def get_user_support_dashboard(self, container_id: str):
        ...
        return {
            "support_requests": {
                "open_tickets": 5,
                "resolved_today": 8,
                "avg_resolution_time": "3.2시간",
                "user_satisfaction": 4.5
            },
            "common_issues": [
                {"issue": "권한 요청", "count": 12, "category": "access"},
                {"issue": "문서 업로드 실패", "count": 8, "category": "technical"},
                {"issue": "검색 결과 부정확", "count": 6, "category": "content"}
            ],
            "training_needs": {
                "identified_gaps": ["고급 검색 사용법", "문서 태깅 방법"],
    ...
    async def create_user_guide(self, container_id: str, guide_type: str):
        ...
        if guide_type == "quick_start":
            return {
                "guide_id": "QG_001",
                "title": f"{await self.get_container_name(container_id)} 빠른 시작 가이드",
                "sections": [
                    "컨테이너 접근 방법",
                    "문서 검색 및 다운로드",
                    "새 문서 업로드",
                ...
                "target_audience": "신규 사용자"
            }
```

### 12.4 지식 품질 및 생애주기 관리
...
async def get_container_hierarchy(
    ...
    try:
        ...
        if container_id:
            ...
            if not has_access:
                raise HTTPException(403, "해당 컨테이너에 접근 권한이 없습니다")
        ...
        logger.error(f"컨테이너 계층 조회 실패: {e}")
...
async def create_container(
    ...
    try:
        ...
        if current_user.role not in ['ADMIN', 'MANAGER']:
            raise HTTPException(403, "컨테이너 생성 권한이 없습니다")
        
        # 2. 상위 컨테이너에 대한 관리 권한 확인
        if container_data.parent_container_id:
            ...
            if not has_admin_permission:
                raise HTTPException(403, "상위 컨테이너에 대한 관리 권한이 없습니다")
...
async def get_container_permissions(
    ...
    """컨테이너 권한 조회"""
    try:
        # 1. 권한 조회 권한 확인 (ADMIN 또는 MANAGER만 가능)
        has_admin_permission = await permission_service.check_container_permission(
            current_user.emp_no, container_id, "ADMIN"
        )
        if not has_admin_permission:
            raise HTTPException(403, "권한 조회 권한이 없습니다")
...
async def request_container_permission(
    ...
    try:
        ...
        if existing_permission and existing_permission["permission_type"] in ["ADMIN", "MANAGER"]:
            return {"message": "이미 충분한 권한을 보유하고 있습니다"}
        ...
        if existing_request:
            return {"message": "이미 권한 요청이 처리 대기 중입니다", "request_id": existing_request["request_id"]}
        ...
        return {
            "request_id": request_id,
            "status": "submitted",
            "estimated_processing_time": "1-3 영업일",
...
async def update_user_permission(
    ...
    try:
        ...
        if not has_admin_permission:
            raise HTTPException(403, "권한 변경 권한이 없습니다")
        
        # 2. 자신의 권한은 변경할 수 없음
        if user_id == current_user.emp_no:
            raise HTTPException(400, "자신의 권한은 변경할 수 없습니다")
        ...
        raise HTTPException(500, f"업데이트 실패: {str(e)}")
```

### 13.3 컨테이너별 사용자 지원 API
...
async def get_container_analytics(
    ...
    try:
        ...
        if not has_view_permission:
            raise HTTPException(403, "해당 컨테이너 분석 조회 권한이 없습니다")
        
        # 2. 관리자 레벨 분석과 일반 사용자 분석 구분
        ...
        raise HTTPException(500, f"분석 조회 실패: {str(e)}")
```

### 13.4 권한 기반 컨테이너 API 엔드포인트 정리

```python
# Container Management APIs (권한 기반)
/api/containers/
├── hierarchy                    # GET: 사용자 접근 가능한 계층 구조
├── create                      # POST: 컨테이너 생성 (ADMIN/MANAGER만)
├── {id}/
│   ├── info                   # GET: 컨테이너 정보 (접근 권한 확인)
│   ├── permissions            # GET: 권한 목록 (ADMIN만)
│   ├── permissions/request    # POST: 권한 요청
│   ├── permissions/{user_id}  # PUT/DELETE: 권한 관리 (ADMIN만)
│   ├── analytics             # GET: 사용 분석 (권한 레벨별 제한)
│   └── documents             # GET: 문서 목록 (접근 권한 기반 필터링)
├── permissions/
│   ├── my-containers         # GET: 내가 접근 가능한 컨테이너 목록
│   ├── requests             # GET: 내 권한 요청 현황
│   └── audit                # GET: 권한 감사 로그 (ADMIN만)
└── admin/
    ├── bulk-permission-update  # POST: 대량 권한 업데이트
    ├── permission-templates    # GET/POST: 권한 템플릿 관리
    └── system-audit           # GET: 시스템 전체 감사 (시스템 관리자만)
```