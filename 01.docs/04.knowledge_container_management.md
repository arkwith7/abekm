# 04. IP 포트폴리오 관리 시스템 (구 지식 컨테이너)

> **최종 업데이트**: 2026-01-03
> **주요 변경**:
> - "지식 컨테이너" 개념을 "IP 포트폴리오"로 확장 및 고도화
> - 조직도 기반 분류에서 **IPC/CPC 기술 분류 체계** 중심으로 전환
> - 특허 문서 특화 메타데이터 및 시각화(특허 카드) 도입

## 1. 시스템 개요

### 1.1 목적

IPBridge의 IP 포트폴리오 관리 시스템은 기존의 조직도 기반 지식 저장소를 넘어, **국제특허분류(IPC) 및 협력특허분류(CPC)** 기반의 체계적인 특허 자산 관리를 지원합니다. 이를 통해 기업의 기술 자산을 표준화된 체계로 분류하고, 경쟁사와의 비교 분석 및 전략적 의사결정을 지원합니다.

### 1.2 IP 포트폴리오 개념

- **기술 분류 컨테이너**: IPC/CPC 코드를 기준으로 한 표준화된 지식 저장 단위 (예: H04W, G06N)
- **커스텀 매핑**: 기업 내부 용어(예: "차세대 배터리")를 표준 IPC 코드와 매핑하여 가상 폴더 제공
- **특허 중심 메타데이터**: 출원번호, 출원일, 법적상태, 청구항 등 특허 특화 속성 관리
- **자동 분류**: 문서 업로드 시 텍스트 분석을 통해 적절한 IPC/CPC 코드로 자동 분류 및 태깅

### 1.3 시스템 범위

- **IPC/CPC 표준 분류 체계 구축**: 8개 섹션 및 하위 클래스 DB 구축
- **특허 문서 카드(Patent Card)**: 썸네일, 법적상태, 핵심 키워드 등을 시각화한 UI
- **자동 분류 엔진**: 텍스트 기반 IPC 추천 및 자동 파일링
- **권한 관리**: 기술 분야별/프로젝트별 접근 제어 (기존 조직 권한과 하이브리드 운용)
- **포트폴리오 시각화**: 기술 분야별 특허 분포 도넛 차트, 연도별 출원 추이 그래프
...
```
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   SAP RFC       │───▶│   Organization  │───▶│   Knowledge     │
│   Integration   │    │   Tree Builder  │    │   Container     │
└─────────────────┘    └─────────────────┘    │   Hierarchy     │
                                 │             └─────────────────┘
                                 ▼                        │
┌─────────────────┐    ┌─────────────────┐               ▼
│   User          │───▶│   Permission    │    ┌─────────────────┐
│   Authentication│    │   Inheritance   │───▶│   Access        │
└─────────────────┘    │   Engine        │    │   Control       │
                       └─────────────────┘    │   Matrix        │
                                 │             └─────────────────┘
                                 ▼                        │
...
│   Request &     │    │   Permission    │    │   Knowledge     │
│   Approval      │    │   Management    │    │   Classification│
└─────────────────┘    └─────────────────┘    └─────────────────┘
                                 │                        │
                                 ▼                        ▼
                       ┌─────────────────┐    ┌─────────────────┐
