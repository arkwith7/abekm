...
> 본 문서는 2025년 12월 현재 운영/구현된 기능 중심의 **실제 시스템 구조와 동작 현황**을 기술합니다.
...
- **Security First**: 모든 문서 접근은 RBAC(Role-Based Access Control) 및 ABAC(Attribute-Based)를 통해 엄격히 통제.
- **Auditability**: 문서의 업로드, 조회, 다운로드 등 모든 활동에 대한 감사 로그 기록.
...
- **(연관 시스템 SW)**:
    ...
    - 검색/리서치: `backend/app/agents/paper_search_agent.py`(SearchAgent), `backend/app/agents/paper_search_agent_v2.py`(자율형 ReAct)
    - 심층리서치: `backend/app/agents/deep_research_agent.py`
    - 특허분석: `backend/app/agents/patent/patent_analysis_agent.py`, `backend/app/agents/patent/patent_analysis_agent_v2.py`(자율형 ReAct)
...
- **(현재 구현)**: 특허 검색/분석 에이전트, 논문/웹/내부지식 검색 에이전트, Deep Research, PPT 생성(Quick/Template)
- **(향후 확장)**: Disclosure 정제·구조화 에이전트, 가치 스크리닝(Valuation) 에이전트, 리포트 생성 에이전트(PDF/HTML) 및 실험/평가 모듈
...
- **주요 기능**: 사용자 관리, 권한 매트릭스 설정, 시스템 통계 대시보드, 문서 강제 삭제/복구.
...
#### 1.3.4 실제 구현된 API 엔드포인트 구조
- `/api/v1/documents`: 업로드, 목록 조회, 삭제.
- `/api/v1/search`: 통합 검색, 벡터 검색.
- `/api/v1/chat`: RAG 대화, 에이전트 대화.
- `/api/v1/admin`: 관리자 전용 기능.

#### 1.3.5 관리자 API 엔드포인트 요약
- 사용자/권한 관리, 시스템 설정(환경변수 일부), 감사 로그 조회, 통계 데이터 조회.

#### 1.3.6 에이전트형 IP Copilot 기능(현재 구현 + 향후 확장)

##### (A) Disclosure 정제·구조화(향후)
...
- 자율형 실행: LangGraph ReAct 기반(`patent_analysis_agent_v2.py`)
- 논문/문서/웹 검색: `paper_search_agent.py` 및 V2(ReAct)
...
- 근거 기반 리포트 초안(향후: PDF/HTML)
- PPT 생성(현재): `unified_presentation_agent.py` (Quick/Template, LangGraph 체크포인트 기반)
...
- `01.docs/12.agent_design_for_patent_analysis.md`: 특허 분석 에이전트 설계(도메인 관점)
