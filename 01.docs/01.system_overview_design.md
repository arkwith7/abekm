# 01. IPBridge IP 관리 솔루션 시스템 개요 설계서

> 본 문서는 IPBridge 시스템의 **목표 아키텍처(청사진)**와 **현재 구현 상태**를 함께 기술합니다.
> 
...
- [1. 어플리케이션 아키텍처(IPBridge Application)](#1-어플리케이션-아키텍처abekm-application)
...
IPBridge 시스템은 **마이크로서비스 지향의 모듈형 아키텍처**를 기반으로 하며, 프론트엔드, 백엔드 API, 비동기 워커, 그리고 다양한 외부 AI 서비스가 유기적으로 결합되어 있습니다.

본 플랫폼은 **Autonomous Agentic AI 기반의 엔터프라이즈 지식관리 및 IP 솔루션**입니다. 두 개의 독립적인 핵심 시스템으로 구성됩니다:

**1. 지식 컨테이너 시스템 (Knowledge Container)**
- 조직도 기반 일반 업무 문서/지식 관리
- SAP ERP 조직도 동기화 연동
- UI 경로: `/containers`
- 상세 문서: [04.knowledge_container_management.md](04.knowledge_container_management.md)

**2. IP 포트폴리오 시스템 (IP Portfolio)**
- IPC/CPC 기술 분류 기반 특허/IP 자산 관리
- 조직 개편과 무관한 국제 표준 기반 관리
- UI 경로: `/ip-portfolio`
- 상세 문서: [17.IP_Portfolio_Complete_Guide.md](17.IP_Portfolio_Complete_Guide.md)

**3. 공통 AI 기능**
- **IP Copilot**: 통합 대화형 인터페이스에서 검색, 분석, 생성 수행
- **선행기술 조사 (Prior Art Search)**: KIPRIS 실시간 검색과 내부 DB 검색을 결합한 하이브리드 분석 및 X-Y 매트릭스 생성
...
IPBridge (AI Based Enterprise Knowledge Management)은 기업 내 산재된 비정형 문서(PDF, Office, HWP 등)를 중앙화하고, 최신 AI 기술(RAG, LLM, Agent)을 적용하여 지식의 **검색, 요약, 재생산**을 자동화하는 것을 목적으로 합니다.
...
## 1. 어플리케이션 아키텍처(IPBridge Application)
...
IPBridge의 Agentic Architecture를 "특허/논문 기반 IP 의사결정" 도메인으로 확장한 서브시스템입니다.
...
KIPRIS 등 외부 특허 DB에서 특허를 자동 수집하고, **IP 포트폴리오 시스템**에서 IPC 기반으로 관리하는 서브시스템입니다. 수집된 특허는 일반 문서 시스템(`TbFileBssInfo`)에도 연동되어 통합 검색이 가능합니다.
...
| `TbFileBssInfo` | 일반 문서 목록 (특허는 `document_type='patent'`) |

##### 일반 문서 시스템 연동 ✅

- 수집된 특허는 `TbFileBssInfo`에 `document_type='patent'`로 저장 (통합 검색용)
- `path` 필드에 외부 URL 저장 (원본 PDF 미저장 정책)
- 특허 메타데이터는 `TbPatentBibliographicInfo`에 별도 저장
- IP 포트폴리오 UI(`/ip-portfolio`)에서 IPC 기반 분류 관리
...
| `/api/v1/containers` | 문서 컨테이너 관리 (조직도 기반) |
| `/api/v1/ip-portfolio` | IP 포트폴리오 관리 (IPC 기반) |
...
IPBridge의 Agentic Runtime은 **LangGraph 기반 Supervisor + 전문 에이전트(검색/특허/리포트/PPT)** 구조를 따릅니다.
...
> 특허 정보 관련 기능은 IPBridge 시스템의 핵심 사업 가치를 담당하는 영역으로, 체계적인 관리와 문서화가 필수적입니다.
...
├── 15.kipris_plus_api_reference.md   # L3: KIPRIS PLUS API 설명서(REST/XML)
