...
본 플랫폼은 **Autonomous Agentic AI 기반의 IP(특허/논문) 솔루션**으로 전환되었습니다. 핵심 성장축은 다음과 같습니다:

1.  **IP 포트폴리오 (IP Portfolio)**: 기존 지식 컨테이너를 IPC/CPC 기술 분류 체계로 고도화
2.  **IP Copilot**: 통합 대화형 인터페이스에서 검색, 분석, 생성 수행
3.  **선행기술 조사 (Prior Art Search)**: KIPRIS 실시간 검색과 내부 DB 검색을 결합한 하이브리드 분석 및 X-Y 매트릭스 생성
