...
- **다중 파이프라인 지원**: 오픈소스, Azure Document Intelligence, AWS Textract
- **멀티모달 처리**: 텍스트, 이미지, 표, 레이아웃 통합 분석
...
### 1.4 최근 주요 업데이트 (2025-10-17)

#### 1.4.1 벡터 차원 통합 (1536차원)

- **배경**: Azure OpenAI `text-embedding-3-small` 모델로 통일
- **마이그레이션**: `vs_doc_contents_chunks.chunk_embedding` → vector(1024) to vector(1536)
- **인덱스**: HNSW 재생성 (m=16, ef_construction=64)
- **Alembic 리비전**: `k3l4m5n6o7p8_migrate_vs_doc_contents_chunks_to_1536_dim.py`
- **설정 파일**: `backend/.env` → `VECTOR_DIMENSION=1536`

#### 1.4.1.1 CLIP 벡터 차원 추가 (512차원) 🆕

- **목적**: 이미지-텍스트 크로스모달 검색 지원
- **모델**: Hugging Face `openai/clip-vit-base-patch32`
- **저장**: `doc_embedding.clip_vector` (vector(512))
- **인덱스**: HNSW 인덱스 생성 (m=16, ef_construction=64)
- **Fallback**: Azure CLIP API → 로컬 CLIP 모델 → Placeholder (모두 실패 시)
- **성능**: L2 Norm = 1.0000 (정규화 완료)

#### 1.4.2 한국어 전문검색 (FTS) 트리거 구현
...
**버킷 + Prefix 구조:**
```
S3 Bucket: wikl-file-bucket-20250910
├── {knowledge_container_id}/raw/{file_physical_name}
...
url = s3_client.generate_presigned_url(
    ...
    Params={
        'Bucket': 'wikl-file-bucket-20250910',
...
aws s3 mb s3://wikl-file-bucket-20250910 --region ap-northeast-2
...
# 7단계: S3 업로드 확인
aws s3 ls s3://wikl-file-bucket-20250910/sales-dept/raw/
```
...
# S3 사용량
aws s3 ls s3://wikl-file-bucket-20250910 --summarize --recursive
```
...
# 3. S3 권한 오류
aws s3 cp test.txt s3://wikl-file-bucket-20250910/ --debug
...
- Azure Document Intelligence: `prebuilt-document` + `prebuilt-layout` 병행 호출, 2025-10-10 기준으로 **그림(figure)** 미검출 시 `pdfplumber` 기반 폴백 추출을 자동 적용하고 페이지 메타데이터에 병합
...
4. **추출 결과 DB 저장** 🆕
   ...
   - **doc_embedding**: 모델별 임베딩 버전 관리 (Azure OpenAI text-embedding-3-small, 1536차원)
5. **검색 인덱스 자동 업데이트** 🆕
   - **vs_doc_contents_chunks**: pgvector HNSW 인덱스 (1536차원)
...
```mermaid
graph TB
    A[문서 업로드] --> B{이미지 PDF 판별}
    B -->|텍스트 PDF| C1[기존 텍스트 추출]
    B -->|이미지 PDF| C2[OCR 엔진 선택]
...
graph TB
    A[사용자 검색 쿼리] --> B{검색 타입}
    B -->|키워드| C[kor_search 확장]
    B -->|의미검색| D[pgvector 검색]
    B -->|하이브리드| E[통합 검색]
