# 🎨 IPBridge 역할별 UI/UX 분리 설계 및 최신 표준 통합
...
```
┌─────────────────────────────────────────────┐
│ IPBridge    [대시보드] [검색] [업로드] [💬] [👤] │ ← 상단 탭 네비게이션
├─────────────────────────────────────────────┤
│ ┌─────────┐ ┌───────────────────────────────┐│
│ │         │ │                               ││
...
IPBridge를 **PC, 태블릿, 스마트폰** 모든 디바이스에서 최적의 사용자 경험을 제공하는 **역할별 특화 인터페이스**로 구현하기 위한 핵심 화면 레이아웃을 정의합니다.
...
```
┌─────────────────────────────────────────────┐
│ IPBridge [탭1] [탭2] [탭3] [탭4]        [👤]   │ ← 상단 탭 네비게이션
├─────────────────────────────────────────────┤
...
clearAllDocumentsOnLogout: () => {
  ...
  try {
    localStorage.removeItem('IPBridge-app-state');  // Zustand persist 저장소
    localStorage.removeItem('pageStates');        // 레거시 키
    localStorage.removeItem('IPBridge_chat_state');
    localStorage.removeItem('IPBridge_agent_chat_state');
  } catch { /* ignore */ }
},
```
...
2. `IPBridge_chat_state`: 채팅 세션 및 대화 상태
