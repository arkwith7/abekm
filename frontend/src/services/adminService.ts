...
export const getResourceUsage = async (period: string = '1h'): Promise<ResourceUsage[]> => {
  const response = await axios.get(`/api/admin/system/resources`, {
    params: { period }
  });
  return response.data;
};
...
export const userManagementAPI = {
  ...
  resetPassword: async (userId: number, newPassword?: string): Promise<PasswordResetResponse> => {
    const response = await axios.post(`/api/v1/users/${userId}/reset-password`, {
  ...
  getUserStats: async (): Promise<{
    ...
    const response = await axios.get(`/api/v1/users/`, { params: { page: 1, size: 100 } });
  ...
  bulkDeleteUsers: async (userIds: number[]): Promise<{
    ...
    const response = await axios.post(`/api/v1/users/bulk-delete`, { user_ids: userIds });
    return response.data;
  },
  ...
  bulkUpdateRole: async (userIds: number[], isAdmin: boolean): Promise<{
    ...
    const response = await axios.post(`/api/v1/users/bulk-update-role`, {
      user_ids: userIds,
      is_admin: isAdmin
    });
    return response.data;
  ...
  getDepartments: async (): Promise<{ code: string; name: string }[]> => {
    const response = await axios.get(`/api/v1/users/filters/departments`);
  ...
  getPositions: async (): Promise<{ code: string; name: string }[]> => {
    const response = await axios.get(`/api/v1/users/filters/positions`);
...
export const updateUserRole = async (roleId: string, roleData: Partial<UserRole>): Promise<UserRole> => {
  const response = await axios.put(`/api/admin/roles/${roleId}`, roleData);
  return response.data;
};
...
export const assignUserRole = async (userId: string, roleId: string): Promise<void> => {
  await axios.post(`/api/admin/users/${userId}/roles`, { role_id: roleId });
};
...
export const updateSystemConfig = async (configData: Partial<SystemConfig>): Promise<SystemConfig> => {
  const response = await axios.put(`/api/admin/system/config`, configData);
  return response.data;
};

export const getAIModelConfig = async (): Promise<any> => {
  const response = await axios.get(`/api/admin/system/ai-config`);
  return response.data;
};

export const updateAIModelConfig = async (configData: any): Promise<any> => {
  const response = await axios.put(`/api/admin/system/ai-config`, configData);
  return response.data;
};
...
export const getFailedLoginAttempts = async (): Promise<SecurityAudit[]> => {
  const response = await axios.get(`/api/admin/security/failed-logins`);
  return response.data;
};

export const getSecurityEvents = async (eventType?: string): Promise<SecurityAudit[]> => {
  const params = eventType ? { event_type: eventType } : {};
  const response = await axios.get(`/api/admin/security/events`, { params });
  return response.data;
};
...
export const unblockIPAddress = async (ipAddress: string): Promise<void> => {
  await axios.delete(`/api/admin/security/block-ip/${ipAddress}`);
};
...
export const restoreBackup = async (backupId: string): Promise<void> => {
  await axios.post(`/api/admin/backup/restore/${backupId}`);
};
...
export const downloadBackup = async (backupId: string): Promise<void> => {
  const response = await axios.get(`/api/admin/backup/download/${backupId}`, {
    responseType: 'blob'
  });
...
export const reindexDatabase = async (): Promise<void> => {
  await axios.post(`/api/admin/database/reindex`);
};

export const getDatabaseStatistics = async (): Promise<any> => {
  const response = await axios.get(`/api/admin/database/statistics`);
  return response.data;
};
...
export const getSystemLogs = async (level?: string, limit: number = 100): Promise<any[]> => {
  ...
  const response = await axios.get(`/api/admin/logs/system`, { params });
  return response.data;
};

export const getErrorLogs = async (limit: number = 100): Promise<any[]> => {
  const response = await axios.get(`/api/admin/logs/errors`, {
    params: { limit }
  });
  return response.data;
};
...
export const clearLogs = async (logType: string): Promise<void> => {
  await axios.delete(`/api/admin/logs/${logType}`);
};

// 통계 및 분석
export const getUsageStatistics = async (period: string = '30d'): Promise<any> => {
  const response = await axios.get(`/api/admin/analytics/usage`, {
    params: { period }
  });
  return response.data;
};
...
export const getStorageAnalytics = async (): Promise<any> => {
  const response = await axios.get(`/api/admin/analytics/storage`);
  return response.data;
};
