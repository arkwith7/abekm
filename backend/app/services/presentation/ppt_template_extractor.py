...
def update_template_shape_names(prs, template_path):
    ...
    try:
        ...
        if modified:
            ...
            print(f"템플릿 파일 업데이트됨: {template_path}")
    except Exception as e:
...
def extract_presentation(path, output_path):
    ...
    # 템플릿 파일에서 텍스트박스에 ID 이름 설정
    update_template_shape_names(prs, path)
    ...
    for idx, slide in enumerate(prs.slides, start=1):
        ...
        for shape in slide.shapes:
            ...
            if (shape.shape_type.name == "TEXT_BOX" and 
                ...
                # element ID 생성
                element_id = f"textbox-{idx-1}-{textbox_index}"
                
                # 실제 shape에 이름 설정 (매핑 시 찾기 위해)
                shape.name = element_id
