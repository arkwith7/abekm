# PPT 템플릿 메타데이터 추출기 (단일 스크립트 스타일)
import json
from pptx impordef get_font_info(run):
    f = run.font
    return {
        "name": f.name,
...
def get_font_info(run):
    ...
    return {
        ...
        "italic": f.italic,
        "underline": f.underline,
        "color": color_to_hex(f.color) if getattr(f, "color", None) else None,
    }
                    ...
                    if text_frame and text_frame.paragraphs:
                        ...
                        if first_para.runs:
                            ...
                            element.update({
                                ...
                                "color": font_info.get("color", "").replace("#", "") if font_info.get("color") else None,
