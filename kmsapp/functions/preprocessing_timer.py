...
preprocessing_timer_bp = func.Blueprint() 
@preprocessing_timer_bp.function_name(name="preprocessing_timer")
@preprocessing_timer_bp.timer_trigger(schedule="0 0/5 22-23,0-11 * * *", 
              arg_name="mytimer",
              run_on_startup=False) 
def preprocessing_timer(mytimer: func.TimerRequest) -> None:
    log_type = f"preprocessing-timer"
    logger = setup_logger(preprocess_type=log_type)

    last_run_time = datetime.datetime.fromisoformat(mytimer.schedule_status['Last'])
    next_run_time = datetime.datetime.fromisoformat(mytimer.schedule_status['Next'])

    timespan_timedelta = next_run_time - last_run_time
    timespan_str = str(round(timespan_timedelta.days * 24 * 60 + timespan_timedelta.seconds / 60))

    logger.info(f"============================== TIME SPAN: {timespan_str}\n")    
    ...
    all_file_info = mysql_instance.select_all_file_info(conn=mysql_conn, cursor=mysql_cursor, time_span = timespan_str)
    ...
    logger.info(f"============================== START PREPROCESSING TIMER PROCESS\n")    
    for file_info in all_file_info:
        ...
        # 처리 기록이 없거나, success 기록이 없는 경우 False
        if len(item_result) == 0:
            preprocessing_result = False
        elif item_result[0]['document']['preprocessing_result'] == 'fail':
            preprocessing_result = False
        else:
        ...
        # bss table에 등록은 되어있으나('N'값), 위의 조건이 False인 경우
        if not file_info['del_yn'] and not preprocessing_result:
            file_result = aisearch_instance_file.get_item(file_psl_nm, select_list = ['*'], is_result=True)
            file_detail_result = aisearch_instance_search.get_item(file_psl_nm, select_list = ['*'], is_result=True)
            if len(file_result) == 0 or len(file_detail_result) == 0:
                preprocessing_process(logger, file_info, as_instance_raw, as_instance_pre, as_instance_log, di_instnace, aisearch_instance_file, aisearch_instance_search, aoai_instance, pre_instance, aisearch_instance_preprocessing_result)
            # elif len(file_detail_result) != 0:
            #     # preprocessing_process(logger, file_info, as_instance_raw, as_instance_pre, as_instance_log, di_instnace, aisearch_instance_file, aisearch_instance_search, aoai_instance, pre_instance, aisearch_instance_preprocessing_result)
            #     try:
            #         save_path = f"DI_result/{file_psl_nm}.json"
            ...
            #     if len(file_detail_result) != len(pages):
            #         preprocessing_process(logger, file_info, as_instance_raw, as_instance_pre, as_instance_log, di_instnace, aisearch_instance_file, aisearch_instance_search, aoai_instance, pre_instance, aisearch_instance_preprocessing_result)
            else:
                continue
        elif not file_info['del_yn'] and preprocessing_result:
            file_result = aisearch_instance_file.get_item(file_psl_nm, select_list = ['*'], is_result=True)
            file_detail_result = aisearch_instance_search.get_item(file_psl_nm, select_list = ['*'], is_result=True)
            if len(file_result) == 0 or len(file_detail_result) == 0:
                preprocessing_process(logger, file_info, as_instance_raw, as_instance_pre, as_instance_log, di_instnace, aisearch_instance_file, aisearch_instance_search, aoai_instance, pre_instance, aisearch_instance_preprocessing_result)

        elif file_info['del_yn']:
            file_result = aisearch_instance_file.get_item(file_psl_nm, select_list = ['*'], is_result=True)
            file_detail_result = aisearch_instance_search.get_item(file_psl_nm, select_list = ['*'], is_result=True)
            ...
            if len(file_result) == 0:
                ...
                if not file_result[0]['document']['del_yn']:
                    logger.info(f'========================{file_result}')
                    # 파일정보 업데이트
                    if len(file_result) > 0:
                        aisearch_instance_file.update_del_yn(file_result[0]['document'])

                    # 상세정보 업데이트
                    if len(file_detail_result) > 0:
                        for file_contents in file_detail_result:
...
def preprocessing_process(logger, file_info, as_instance_raw, as_instance_pre, as_instance_log, di_instnace, aisearch_instance_file, aisearch_instance_search, aoai_instance, pre_instance, aisearch_instance_preprocessing_result):
    log_type = f"preprocessing-timer"
    logger = setup_logger(preprocess_type=log_type)
    ...
    logging.info('Python Blob trigger function processed %s', file_info['file_lgc_nm'])
    # logging.info('Python Blob trigger function processed %s', blobFile.name)

    # file_path = file_info['path'][1:]+'/'+file_psl_nm

    file_name = file_info['file_psl_nm']
    file_type = file_info['file_extsn']

    file_path = file_info['path'][1:]+'/'+file_name+'.'+file_type
    ...
    try :
        ...
        # file_info['file_lgc_nm'] = file_info['file_lgc_nm']
